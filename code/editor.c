internal void
add_editor_newline(EditorGroup *group, u32 count)
{
    assert(group);
    assert(count);
...
#define add_editor_source_entity(group, id) add_editor_source(group, EditorSourceType_Entity, id, 0, 0, get_entity_name(id))
internal void
add_editor_source(EditorGroup *group, EditorSourceType type, u32 subtype, ItemState *item_state, ItemRarity item_rarity, char *name)
{
    assert(group);
    assert(type);
    assert(subtype);
    ...
    for(u32 index = 0; index < MAX_EDITOR_SOURCE_COUNT; ++index)
    {
        ...
        if(!source->id)
        {
            ...
            switch(type)
            {
                ...
                {
                    source->tile_src = get_dungeon_tileset_rect(subtype);
                } break;
                ...
                {
                    source->tile_src = get_dungeon_trap_tile_src(subtype);
                } break;
                
                case EditorSourceType_Item:
                {
                    source->item_rarity = item_rarity;
                    source->tile_src = get_item_tileset_rect(item_state, subtype, item_rarity, false);
                } break;
                
                case EditorSourceType_Entity:
                {
                    source->tile_src = get_dungeon_tile_rect_from_entity_id(subtype);
                } break;
            ...
            // Update pos
            ++group->pos.x;
            if(group->sources_per_row &&
               group->pos.x >= group->start_pos.x + group->sources_per_row)
            {
                add_editor_newline(group, 1);
            }
                    ...
                    EditorSource *source)
{
    switch(source->type)
    {
        ...
        case EditorSourceType_DungeonTile:
        {
            set_dungeon_pos_tile(dungeon->tiles, mouse_tile_pos, source->subtype);
        } break;
        ...
        {
            assert(source->subtype);
            assert(source->item_rarity);
            
            if(!get_dungeon_pos_item(item_state, dungeon->level, mouse_tile_pos, ItemID_None))
            {
                add_item(random, item_state, dungeon->level, source->subtype, source->item_rarity, mouse_tile_pos.x, mouse_tile_pos.y, false, 1);
            }
        } break;
        ...
        case EditorSourceType_Entity:
        {
            ...
            {
                add_entity(entity_state, source->subtype, mouse_tile_pos.x, mouse_tile_pos.y, dungeon);
            }
        } break;
...
get_editor_target_pos(EditorTarget *target)
{
    ...
    switch(target->type)
    {
        ...
        case EditorTargetType_Item: result = target->item->pos; break;
                     ...
                     EditorTarget *target)
{
    ...
    else
    {
        ...
        {
            Item *item = get_dungeon_pos_item(item_state, dungeon->level, mouse_tile_pos, ItemID_None);
            if(item)
            {
                zero_editor_target_if_on_pos(target, item->pos);
                remove_item(item, item_owners);
            }
            else
            {
                  ...
                  v2u mouse_tile_pos)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if(item)
            {
                target->type = EditorTargetType_Item;
                target->item = item;
            }
                     ...
                     EditorMode *editor)
{
    ...
    else if(target->item)
    {
        ...
        {
            target->item->pos = mouse_tile_pos;
        }
    }
    else if(tile->id)
    {
                   ...
                   Dungeon *dungeon)
{
    if(editor->is_open)
    {
        ...
        if(!updated_mark_input)
        {
            ...
            if(was_pressed(&input->Button_Left))
            {
                ...
                if(!updated_mark)
                {
                    ...
                    if(!updated_sources)
                    {
                        ...
                        if(source)
                        {
                            place_editor_source(random, entity_state, item_state, dungeon, input->mouse_tile_pos, source);
                        }
                        else if(target->type)
                        {
                            update_editor_target(random, entity_state, item_state, dungeon, input->mouse_tile_pos, target, editor);
                        }
                        else
                        {
                            set_editor_target(editor, entity_state, item_state, dungeon, input->mouse_tile_pos);
                        }
                    }
                }
            }
            ...
            {
                delete_editor_target(random, entity_state, item_state, item_owners, dungeon, input->mouse_tile_pos, target);
            }
        }
    }
}
                   ...
                   Texture tileset)
{
    if(editor->is_open)
    {
        ...
        {
            v2u target_pos = get_editor_target_pos(target);
            v4u rect = get_game_dest(camera.rect, target_pos);
