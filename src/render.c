void render_level()
{
  SDL_SetRenderTarget(game_state.renderer, game_state.assets.textures[tex_tilemap]);
  ...
  for(i32 y = tile_div(game_state.camera.y); y < to_y; y++)
  {
    for(i32 x = tile_div(game_state.camera.x); x < to_x; x++)
    {
      ...
      SDL_RenderCopy(game_state.renderer, game_state.assets.textures[tex_game_tileset], &src, &dst);
    }
  }
  ...
  SDL_RenderCopy(game_state.renderer, game_state.assets.textures[tex_tilemap], &game_state.camera, &dst);
}
...
void render_text(char *txt, iv2_t p, SDL_Color color, font_t *font, ...)
{
  ...
  while(*at)
  {
    ...
    SDL_Rect src = {glyph_metrics->pos.x, glyph_metrics->pos.y, glyph_metrics->aspect.w, glyph_metrics->aspect.h};
